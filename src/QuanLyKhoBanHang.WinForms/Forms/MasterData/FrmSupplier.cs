...
public sealed class FrmSupplier : CrudListForm<SupplierDto>
{
    ...
    public FrmSupplier() : base("Nhà cung cấp", "Quản lý thông tin nhà cung cấp, liên hệ và trạng thái hoạt động.")
    {
    ...
    protected override Control BuildHeader(string title, string subtitle)
    {
        ...
        header.Controls.Add(UiFactory.SectionHeader(title, subtitle, IconChar.Truck), 0, 0);
    ...
    protected override void ApplyFilter()
    {
        ...
        _filteredItems = string.IsNullOrWhiteSpace(keyword)
            ...
            : _items.Where(p =>
                    ...
                    (p.Phone ?? string.Empty).Contains(keyword, StringComparison.OrdinalIgnoreCase) ||
                    (p.Email ?? string.Empty).Contains(keyword, StringComparison.OrdinalIgnoreCase) ||
                    (p.Address ?? string.Empty).Contains(keyword, StringComparison.OrdinalIgnoreCase))
                .ToList();
    ...
    {
        _selectedItem = BindingSource.Current as SupplierDto;
        if (_selectedItem is null)
        {
            ...
            _selectedStateLabel.Text = "Chưa chọn nhà cung cấp";
        ...
        _editModeLabel.Text = _selectedItem.IsActive ? "Nhà cung cấp đang hoạt động." : "Nhà cung cấp đang tạm ngừng.";
        UpdateSelectionState();
    }
    ...
    protected override void BeginAdd()
    {
        ...
        ToggleEditing(true);
        _selectedStateLabel.Text = "Đang tạo nhà cung cấp mới";
        _editModeLabel.Text = "Nhập thông tin rồi bấm Lưu.";
        UiFactory.SetMessage(_gridStateLabel, "Sẵn sàng nhập nhà cung cấp mới.");
        UpdateSelectionState();
    }
    ...
    protected override void BeginEdit()
    {
        ...
        {
            UiFactory.SetMessage(_gridStateLabel, "Vui lòng chọn một nhà cung cấp để sửa.", true);
            return;
        }
        ...
        UiFactory.SetMessage(_gridStateLabel, "Đang chỉnh sửa nhà cung cấp đã chọn.");
        UpdateSelectionState();
    }
    ...
    protected override void SaveCurrent()
    {
        ...
        {
            UiFactory.SetMessage(_gridStateLabel, "Mã và tên nhà cung cấp là bắt buộc.", true);
            return;
        }
        ...
        if (SelectedId <= 0)
        {
            var result = _service.CreateSupplier(dto);
            if (result.Success)
            {
                ToggleEditing(false);
                RefreshData();
    ...
    protected override void DeactivateCurrent()
    {
        if (SelectedId <= 0)
        {
            UiFactory.SetMessage(_gridStateLabel, "Vui lòng chọn một nhà cung cấp để ngừng kích hoạt.", true);
            return;
        }

        var result = _service.DeactivateSupplier(SelectedId);
        if (result.Success)
        {
            RefreshData();
            UiFactory.SetMessage(_gridStateLabel, result.Message);
    ...
    private void ConfigureScreen()
    {
        ...
        root.Controls.Clear();
        root.Controls.Add(BuildHeader("Đối tác cung ứng", "Quản lý thông tin liên hệ, địa chỉ và trạng thái nhà cung cấp."), 0, 0);
    ...
    private Control BuildGridHeader()
    {
        ...
        layout.Controls.Add(UiFactory.SectionHeader("Danh sách nhà cung cấp", "Bảng dữ liệu ưu tiên đọc nhanh, lọc nhanh và chọn dòng rõ ràng.", IconChar.Truck), 0, 0);
        card.Controls.Add(layout);
        return card;
    }
    ...
    private void ConfigureGridCard()
    {
        ...
        Grid.Columns.Clear();
        Grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = nameof(SupplierDto.Code), HeaderText = "Mã NCC", FillWeight = 82, MinimumWidth = 86 });
        Grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = nameof(SupplierDto.Name), HeaderText = "Tên nhà cung cấp", FillWeight = 150, MinimumWidth = 160 });
        Grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = nameof(SupplierDto.Phone), HeaderText = "Điện thoại", FillWeight = 105, MinimumWidth = 108 });
        Grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = nameof(SupplierDto.Email), HeaderText = "Email", FillWeight = 145, MinimumWidth = 140 });
        Grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = nameof(SupplierDto.Address), HeaderText = "Địa chỉ", FillWeight = 150, MinimumWidth = 140 });
        Grid.Columns.Add(new DataGridViewCheckBoxColumn { DataPropertyName = nameof(SupplierDto.IsActive), HeaderText = "Hoạt động", FillWeight = 78, MinimumWidth = 96 });
    ...
    private void ConfigureEditCard()
    {
        ...
        ConfigureField(CodeBox, 0, "Mã NCC");
        ConfigureField(NameBox, 1, "Tên nhà cung cấp");
        ...
        wrapper.RowStyles.Add(new RowStyle(SizeType.Absolute, 34));
        wrapper.Controls.Add(UiFactory.SectionHeader("Chi tiết nhà cung cấp", "Thông tin nhập liệu và trạng thái", IconChar.Truck), 0, 0);
    ...
    private Control BuildQuickMetrics()
    {
        ...
        metrics.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 33.33F));
        metrics.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 33.33F));
        metrics.Controls.Add(UiFactory.MetricCard("Tổng NCC", _metricTotalValue, IconChar.Truck, AppTheme.Primary, AppTheme.PrimarySoft), 0, 0);
    ...
    private void UpdateSummary()
    {
        ...
        _summaryLabel.Text = $"{_items.Count} nhà cung cấp | {active} đang hoạt động";
    }
    ...
    private void UpdateGridState()
    {
        ...
        EmptyPanel.Controls.Clear();
        _emptyTitleLabel.Text = "Không tìm thấy nhà cung cấp phù hợp";
        ...
        var iconTile = UiFactory.IconTile(IconChar.Truck, AppTheme.Primary, AppTheme.PrimarySoft, 64, 28);
        ...
        layout.Controls.Add(new Label
        {
            Text = "Mẹo: dùng mã NCC, tên, điện thoại, email hoặc địa chỉ để lọc nhanh.",
            Dock = DockStyle.Fill,
            ForeColor = AppTheme.TextMuted,
    ...
    [
        ...
        new SupplierDto { Id = 3, Code = "NCC-03", Name = "Vận tải Toàn Cầu", Phone = "0988776655", Email = "toancau@example.com", Address = "Hà Nội", IsActive = false }
    ];
