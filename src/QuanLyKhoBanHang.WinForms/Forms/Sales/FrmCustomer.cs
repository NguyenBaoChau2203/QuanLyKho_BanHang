...
public sealed class FrmCustomer : CrudListForm<CustomerDto>
{
    ...
    public FrmCustomer() : base("Khách hàng", "Quản lý khách hàng, thông tin liên hệ và trạng thái hoạt động.")
    {
        ...
        _setReadonlyButton = UiFactory.IconActionButton("Chỉ xem", IconChar.Eye, (_, _) => { ToggleEditing(false); UpdateSelectionState(); }, 88);
    ...
    protected override Control BuildHeader(string title, string subtitle)
    {
        ...
        header.Controls.Add(UiFactory.SectionHeader(title, subtitle, IconChar.Users), 0, 0);
    ...
    protected override void ApplyFilter()
    {
        ...
        _filteredItems = string.IsNullOrWhiteSpace(keyword)
            ? _items.ToList()
            : _items.Where(customer =>
                    customer.Code.Contains(keyword, StringComparison.OrdinalIgnoreCase) ||
                    customer.Name.Contains(keyword, StringComparison.OrdinalIgnoreCase) ||
                    (customer.Phone ?? string.Empty).Contains(keyword, StringComparison.OrdinalIgnoreCase) ||
                    (customer.Email ?? string.Empty).Contains(keyword, StringComparison.OrdinalIgnoreCase) ||
                    (customer.Address ?? string.Empty).Contains(keyword, StringComparison.OrdinalIgnoreCase))
                .ToList();
    ...
    {
        _selectedItem = BindingSource.Current as CustomerDto;
        if (_selectedItem is null)
        {
            ...
            _selectedStateLabel.Text = "Chưa chọn khách hàng";
        ...
        _phoneBox.Text = _selectedItem.Phone ?? string.Empty;
        _emailBox.Text = _selectedItem.Email ?? string.Empty;
        _addressBox.Text = _selectedItem.Address ?? string.Empty;
        ActiveBox.Checked = _selectedItem.IsActive;
        _selectedStateLabel.Text = $"Đã chọn: {_selectedItem.Code} - {_selectedItem.Name}";
        _editModeLabel.Text = _selectedItem.IsActive ? "Khách hàng đang hoạt động." : "Khách hàng đang tạm ngừng.";
        UpdateSelectionState();
    }
    ...
    protected override void BeginAdd()
    {
        ...
        ToggleEditing(true);
        _selectedStateLabel.Text = "Đang tạo khách hàng mới";
        _editModeLabel.Text = "Nhập thông tin rồi bấm Lưu.";
        UiFactory.SetMessage(_gridStateLabel, "Sẵn sàng nhập khách hàng mới.");
        UpdateSelectionState();
    }
    ...
    protected override void BeginEdit()
    {
        ...
        {
            UiFactory.SetMessage(_gridStateLabel, "Vui lòng chọn một khách hàng để sửa.", true);
            return;
        }
        ...
        UiFactory.SetMessage(_gridStateLabel, "Đang chỉnh sửa khách hàng đã chọn.");
        UpdateSelectionState();
    }
    ...
    protected override void SaveCurrent()
    {
        ...
        {
            UiFactory.SetMessage(_gridStateLabel, "Mã và tên khách hàng là bắt buộc.", true);
            return;
        }
    ...
    protected override void DeactivateCurrent()
    {
        if (SelectedId <= 0)
        {
            UiFactory.SetMessage(_gridStateLabel, "Vui lòng chọn một khách hàng để ngừng kích hoạt.", true);
            return;
        }
    ...
    private void ConfigureScreen()
    {
        ...
        root.Controls.Clear();
        root.Controls.Add(BuildHeader("Hồ sơ khách hàng", "Theo dõi thông tin liên hệ và trạng thái khách hàng."), 0, 0);
    ...
    private Control BuildEditorActions() => UiFactory.ActionButtonGrid(
        _addButton,
        _editButton,
        _saveButton,
        _cancelButton,
    ...
    private Control BuildGridHeader()
    {
        ...
        layout.Controls.Add(UiFactory.SectionHeader("Danh sách khách hàng", "Bảng dữ liệu ưu tiên đọc nhanh, lọc nhanh và chọn dòng rõ ràng.", IconChar.Users), 0, 0);
        card.Controls.Add(layout);
        return card;
    }
    ...
    private void ConfigureSearchBar()
    {
        ...
        SearchBox.PlaceholderText = "Tìm theo mã, tên, điện thoại, email, địa chỉ...";
        ...
        _searchHintLabel.Text = "Lọc theo mã, tên, điện thoại, email hoặc địa chỉ.";
    ...
    private void ConfigureGridCard()
    {
        ...
        Grid.Columns.Clear();
        Grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = nameof(CustomerDto.Code), HeaderText = "Mã KH", FillWeight = 82, MinimumWidth = 86 });
        Grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = nameof(CustomerDto.Name), HeaderText = "Tên khách hàng", FillWeight = 150, MinimumWidth = 160 });
        Grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = nameof(CustomerDto.Phone), HeaderText = "Điện thoại", FillWeight = 105, MinimumWidth = 108 });
        Grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = nameof(CustomerDto.Email), HeaderText = "Email", FillWeight = 145, MinimumWidth = 140 });
        Grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = nameof(CustomerDto.Address), HeaderText = "Địa chỉ", FillWeight = 150, MinimumWidth = 140 });
        Grid.Columns.Add(new DataGridViewCheckBoxColumn { DataPropertyName = nameof(CustomerDto.IsActive), HeaderText = "Hoạt động", FillWeight = 78, MinimumWidth = 96 });
    ...
    private void ConfigureEditCard()
    {
        ...
        ConfigureField(CodeBox, 0, "Mã KH");
        ConfigureField(NameBox, 1, "Tên khách hàng");
        ConfigureField(_phoneBox, 2, "Điện thoại");
        ConfigureField(_emailBox, 3, "Email");
        ConfigureField(_addressBox, 4, "Địa chỉ");

        ActiveBox.Text = "Đang hoạt động";
        ActiveBox.AutoSize = true;
        ActiveBox.Margin = new Padding(0, 10, 0, 0);
        _editGrid.Controls.Add(ActiveBox, 0, 5);
        ...
        wrapper.RowStyles.Add(new RowStyle(SizeType.Absolute, 34));
        wrapper.Controls.Add(UiFactory.SectionHeader("Chi tiết khách hàng", "Thông tin liên hệ và trạng thái", IconChar.Users), 0, 0);
    ...
    private void ConfigureField(Control control, int row, string labelText)
    {
        var panel = new Panel
        {
            Dock = DockStyle.Fill,
        ...
        if (control is TextBox box)
        {
            ...
            {
                "Mã KH" => "VD: KH001",
                "Tên khách hàng" => "VD: Cửa hàng An Phú",
                "Điện thoại" => "VD: 0909123456",
                "Email" => "VD: khachhang@example.com",
                "Địa chỉ" => "VD: TP.HCM, Bình Dương...",
                _ => "VD: Nhập dữ liệu..."
            };
            box.Multiline = false;
    ...
    private Control BuildQuickMetrics()
    {
        ...
        metrics.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 33.33F));
        metrics.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 33.33F));
        metrics.Controls.Add(UiFactory.MetricCard("Tổng khách hàng", _metricTotalValue, IconChar.Users, AppTheme.Primary, AppTheme.PrimarySoft), 0, 0);
    ...
    {
        var active = _items.Count(customer => customer.IsActive);
        _summaryLabel.Text = $"{_items.Count} khách hàng | {active} đang hoạt động";
    }

    private void UpdateMetrics()
    {
        var active = _filteredItems.Count(customer => customer.IsActive);
    ...
    {
        var hasKeyword = !string.IsNullOrWhiteSpace(SearchBox.Text);
        var keyword = SearchBox.Text.Trim();
    ...
    private void UpdateGridState()
    {
        ...
        EmptyPanel.Controls.Clear();
        _emptyTitleLabel.Text = "Không tìm thấy khách hàng phù hợp";
        ...
        var iconTile = UiFactory.IconTile(IconChar.Users, AppTheme.Primary, AppTheme.PrimarySoft, 64, 28);
        ...
        layout.Controls.Add(new Label
        {
            Text = "Mẹo: dùng mã KH, tên, điện thoại, email hoặc địa chỉ để lọc nhanh.",
            Dock = DockStyle.Fill,
            ForeColor = AppTheme.TextMuted,
    ...
    private void UpdateSelectionState()
    {
        ...
        _phoneBox.ReadOnly = !IsEditing;
        _emailBox.ReadOnly = !IsEditing;
        _addressBox.ReadOnly = !IsEditing;
    ...
    [
        new CustomerDto { Id = 1, Code = "KH001", Name = "Khách lẻ", Phone = "", Email = "", Address = "", IsActive = true },
